...
test("standalone full-eval, plugin verify/package, and golden paths work in a clean environment", () => {
  ...
  try {
    ...
    for (const run of [full, verify, packaged, golden]) assert.doesNotMatch(run.stdout + run.stderr, /\/$bunfs\/references|Unknown option --workspace|vendor[\\/]node_modules/);
  } finally { rmSync(home, { recursive: true, force: true }); }
});
...
test("release assembly is a clean deterministic projection and never mutates source skills", () => {
  ...
  try {
    ...
    for (const name of expectedNames) {
      ...
      for (const forbidden of ["dist", "package.json", "tests", "tsconfig.json", "vendor"]) assert.equal(statSync(join(skill, forbidden), { throwIfNoEntry: false }), undefined, name + ": " + forbidden);
    }
