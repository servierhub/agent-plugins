...
`make install` independently installs validated staging and accepts `SOURCE=/staged/path`; it defaults safely to the current-target stage and `.agents/plugins/agent-plugins`. For the local build-and-install journey, `make install-local` finishes a fresh `bundle` before installing its default stage, even with `-j`; it intentionally ignores `SOURCE`. Both install paths accept `DEST=/explicit/path`, `DRY_RUN=1`, and `FORCE=1`. Existing destinations are refused unless forced; symlinked destinations or ancestors are always refused.
