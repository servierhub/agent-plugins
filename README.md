...
`make install-plugin` independently installs the validated full plugin staging tree and accepts `SOURCE=/staged/path`; it defaults exactly to the current-target stage and the project-local `.agents/plugins/agent-plugins` destination. For the local build-and-install journey, `make bundle-install-plugin` finishes a fresh `bundle` before installing its default stage, even with `-j`; it intentionally ignores `SOURCE`. Both plugin install paths accept `DEST=/explicit/path`, `DRY_RUN=1`, and `FORCE=1`. Existing destinations are refused unless forced; symlinked destinations or ancestors are always refused. The matching package script is `bun run install:plugin -- ...`.

These commands install a **full plugin** under `.agents/plugins`; they are not standalone Skill installers. A standalone `.skill` artifact belongs under `.agents/skills`, using the host-specific mechanism described above. The compatibility aliases `make install-plugin-local`, `make install-local`, and `make install` remain temporarily available and print deprecation warnings. The two local aliases delegate to `bundle-install-plugin`; `install` delegates to `install-plugin`.
