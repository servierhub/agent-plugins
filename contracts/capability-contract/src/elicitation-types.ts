...
export const ADAPTIVE_ELICITATION_VERSION = "1.0.0" as const;
export const KNOWLEDGE_STATES = ["known", "assumed", "unknown", "contradictory"] as const;
export const DECISION_KINDS = ["architecture", "safety", "evaluation"] as const;
export const AMBIGUITY_CLASSES = ["ordinary", "destructive", "security", "production", "untestable"] as const;
...
export interface AdaptiveElicitationResultV1 {
  ...
  model: { fields: ElicitationFieldV1[] };
  questions: ElicitationQuestionV1[];
  deferredMandatoryFieldIds: string[];
  preview: {
    ready: boolean;
    unansweredOptionalFieldIds: string[];
    appliedDefaults: Array<{ fieldId: string; value: string; consequence: string; reversible: true }>;
    blockers: string[];
  };
}
