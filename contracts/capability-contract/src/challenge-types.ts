...
export const CONTRACT_SCENARIO_CHALLENGE_VERSION = "1.0.0" as const;
export const CHALLENGE_ROLES = ["domain", "ux", "safety", "evaluation"] as const;
export const CHALLENGE_RISKS = ["low", "medium", "high"] as const;
...
export interface ChallengeScenarioV1 { id: string; description: string; }
export type ChallengeCitationV1 =
  | { kind: "contract-field"; path: string }
  | { kind: "scenario"; scenarioId: string };
export interface ChallengeChangeV1 { path: string; value: unknown; }
...
export interface ChallengeRoleInputV1 { role: ChallengeRole; findings: ChallengeFindingV1[]; }
export interface ContractScenarioChallengeRequestV1 {
  ...
  decisions?: Array<{ findingId: string; decision: "accepted" | "rejected" }>;
}
...
export interface ContractScenarioChallengeResultV1 {
  ...
  scenario: { previousHash: string; hash: string | null; invalidated: boolean };
}
