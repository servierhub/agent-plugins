...
export declare const HOST_ADAPTER_PROTOCOL_VERSION: "1.0.0";
export declare const HOST_ADAPTER_SCHEMA_ID: "https://agent-plugins.org/schemas/host-execution-adapter/1.0.0/host-execution-adapter.schema.json";
export declare const HOST_EVENT_SCHEMA_ID: "https://agent-plugins.org/schemas/host-execution-adapter/1.0.0/host-execution-event.schema.json";
...
export interface HostCapabilityRequirements {
    required: HostCapabilityRequirement[];
    optional: HostCapabilityRequirement[];
}
export interface HostCapabilityDegradation {
    capability: HostCapabilityName;
    code: "OPTIONAL_CAPABILITY_UNAVAILABLE";
    message: string;
}
export interface HostNegotiationRequest {
    supportedProtocolVersions: string[];
    capabilities: HostCapabilityRequirements;
}
export interface HostNegotiationAccepted {
    compatible: true;
    protocolVersion: HostAdapterProtocolVersion;
    host: HostIdentity;
    report: HostCapabilityReport;
    degradations: HostCapabilityDegradation[];
}
export interface HostNegotiationBlocked {
    compatible: false;
    protocolVersion?: HostAdapterProtocolVersion;
    host: HostIdentity;
    report: HostCapabilityReport;
...
export interface HostCredentialReference {
    name: string;
    reference: string;
}
export interface HostRunIdentity {
    runId: string;
    planId: string;
    scenarioId: string;
...
export interface HostIdempotency {
    key: string;
    resumeToken?: string;
...
export interface HostRunRequest {
    ...
    workspace: {
        root: string;
        cwd: string;
        files: HostFileBinding[];
    };
    tools: string[];
    environment: {
        names: string[];
        credentials: HostCredentialReference[];
    };
    model: string;
...
export interface HostArtifactExchange {
    protocolVersion: HostAdapterProtocolVersion;
    host: HostIdentity;
    runId: string;
...
export interface HostUsageMetrics {
    inputTokens?: number;
    outputTokens?: number;
    totalTokens?: number;
    costUsd?: number;
...
export interface HostAdapterErrorData {
    code: HostAdapterErrorCode;
    message: string;
    retryable: boolean;
    details?: Record<string, unknown>;
}
export declare class HostAdapterError extends Error {
    readonly data: HostAdapterErrorData;
    constructor(data: HostAdapterErrorData);
}
export interface HostCancellationRequest {
    protocolVersion: HostAdapterProtocolVersion;
    host: HostIdentity;
    runId: string;
    reason: string;
...
export interface HostCancellationResponse {
    protocolVersion: HostAdapterProtocolVersion;
    host: HostIdentity;
    runId: string;
    status: "cancel-requested" | "already-cancelled" | "already-terminal";
}
...
export interface HostArtifactRequest {
    protocolVersion: HostAdapterProtocolVersion;
    host: HostIdentity;
    runId: string;
    artifactId: string;
