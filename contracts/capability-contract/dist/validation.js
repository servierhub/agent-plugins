import { CAPABILITY_CONTRACT_VERSION, } from "./types.js";
...
function validateKnownFields(value, allowed, path, diagnostics) {
    const known = new Set(allowed);
    for (const key of Object.keys(value).sort()) {
        if (!known.has(key))
            diagnostics.push(diagnostic("CAPABILITY_UNKNOWN_FIELD", path + "/" + key, "Unknown field is not allowed: " + key + ".", "Remove " + key + " or move future extension data to the contract root under preserve policy."));
    }
}
function validateStringArray(value, path, diagnostics) {
    if (!Array.isArray(value) || value.some((item) => !nonBlank(item)))
        diagnostics.push(invalid(path, "an array of non-empty strings"));
}
function validateUsers(value, diagnostics) {
    ...
    if (Array.isArray(value))
        value.forEach((item, index) => { if (isRecord(item))
            validateKnownFields(item, ["name", "need"], "/users/" + index, diagnostics); });
}
function validateValues(value, path, diagnostics) {
    ...
    if (Array.isArray(value))
        value.forEach((item, index) => { if (isRecord(item))
            validateKnownFields(item, ["name", "description", "required", "format"], path + "/" + index, diagnostics); });
}
export function validateCapabilityContract(value) {
    ...
    if (isRecord(compatibility))
        validateKnownFields(compatibility, ["unknownFields"], "/compatibility", diagnostics);
    ...
    if (isRecord(value.artifactRecommendation))
        validateKnownFields(value.artifactRecommendation, ["candidateType", "rationale"], "/artifactRecommendation", diagnostics);
    if (!isRecord(value.artifactRecommendation) || !CANDIDATE_TYPES.has(value.artifactRecommendation.candidateType) || (value.artifactRecommendation.rationale !== undefined && !nonBlank(value.artifactRecommendation.rationale))) {
    ...
    if (!isRecord(sideEffects) || typeof sideEffects.applicable !== "boolean") {
        ...
        if (effects !== undefined && !Array.isArray(effects)) {
            ...
            if (!sideEffects.applicable && effectList.length > 0)
                diagnostics.push(invalid("/sideEffects/effects", "empty when sideEffects.applicable is false"));
            if (effectList.some((effect) => !isRecord(effect) || !SIDE_EFFECT_KINDS.has(effect.kind) || !nonBlank(effect.description) || (effect.mitigation !== undefined && !nonBlank(effect.mitigation)))) {
                diagnostics.push(invalid("/sideEffects/effects", "effects with a supported kind and non-empty description"));
            }
            effectList.forEach((effect, index) => { if (isRecord(effect))
                validateKnownFields(effect, ["kind", "description", "mitigation"], "/sideEffects/effects/" + index, diagnostics); });
        }
    }
    ...
    if (!Array.isArray(successSignals) || successSignals.length === 0) {
        ...
        if (successSignals.some((signal) => !isRecord(signal) || !nonBlank(signal.name) || !nonBlank(signal.metric) || !SUCCESS_OPERATORS.has(signal.operator) || typeof signal.target !== "number" || !Number.isFinite(signal.target) || (signal.unit !== undefined && !nonBlank(signal.unit)))) {
            diagnostics.push(invalid("/successSignals", "measurable signals with name, metric, operator, and finite numeric target"));
        }
        successSignals.forEach((signal, index) => { if (isRecord(signal))
            validateKnownFields(signal, ["name", "metric", "operator", "target", "unit"], "/successSignals/" + index, diagnostics); });
    }
    const boundary = value.productionBoundary;
    if (!isRecord(boundary) || !PRODUCTION_LEVELS.has(boundary.level) || !nonBlank(boundary.statement)) {
        ...
        if (boundary.conditions !== undefined)
            validateStringArray(boundary.conditions, "/productionBoundary/conditions", diagnostics);
        if (boundary.excludedUses !== undefined)
            validateStringArray(boundary.excludedUses, "/productionBoundary/excludedUses", diagnostics);
    ...
    if (value.users !== undefined)
        validateUsers(value.users, diagnostics);
    if (value.targetTasks !== undefined)
        validateStringArray(value.targetTasks, "/targetTasks", diagnostics);
    if (value.inputs !== undefined)
        validateValues(value.inputs, "/inputs", diagnostics);
    if (value.outputs !== undefined)
        validateValues(value.outputs, "/outputs", diagnostics);
    for (const field of ["constraints", "assumptions", "risks", "targetHosts"]) {
        if (value[field] !== undefined)
            validateStringArray(value[field], "/" + field, diagnostics);
    }
    if (isRecord(compatibility) && compatibility.unknownFields === "reject") {
        for (const key of Object.keys(value).sort()) {
            if (!KNOWN_ROOT_FIELDS.has(key))
                diagnostics.push(diagnostic("CAPABILITY_UNKNOWN_FIELD", "/" + key, "Unknown root field is not allowed by the reject policy: " + key + ".", "Remove " + key + " or explicitly use compatibility.unknownFields preserve."));
        }
    }
    return { valid: diagnostics.length === 0, diagnostics };
}
const strings = (value) => Array.isArray(value) ? value.map((item) => item.trim()) : [];
...
export function normalizeCapabilityContract(value) {
    const result = validateCapabilityContract(value);
    if (!result.valid)
        throw new CapabilityContractValidationError(result.diagnostics);
    const source = value;
    ...
    return {
        ...
        users: (source.users ?? []).map((user) => ({ name: user.name.trim(), ...(user.need === undefined ? {} : { need: user.need.trim() }) })),
        targetTasks: strings(source.targetTasks),
        artifactRecommendation: { ...source.artifactRecommendation },
        inputs: (source.inputs ?? []).map((item) => ({ ...item, name: item.name.trim(), description: item.description.trim(), ...(item.format === undefined ? {} : { format: item.format.trim() }) })),
        outputs: (source.outputs ?? []).map((item) => ({ ...item, name: item.name.trim(), description: item.description.trim(), ...(item.format === undefined ? {} : { format: item.format.trim() }) })),
        sideEffects: {
            ...source.sideEffects,
            effects: [...(source.sideEffects.effects ?? [])].map((effect) => ({ ...effect })),
        },
        constraints: strings(source.constraints), assumptions: strings(source.assumptions), risks: strings(source.risks),
        successSignals: source.successSignals.map((signal) => ({ ...signal })),
        targetHosts: strings(source.targetHosts),
        productionBoundary: {
            ...source.productionBoundary,
            conditions: strings(source.productionBoundary.conditions),
            excludedUses: strings(source.productionBoundary.excludedUses),
        },
    };
}
