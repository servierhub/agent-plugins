import { ARTIFACT_RECOMMENDATION_VERSION, ARTIFACT_TYPES, } from "./recommendation-types.js";
const RULES = [
...
function assertRequest(value) {
    ...
    for (const key of Object.keys(input))
        if (!["version", "outcome", "explicitType"].includes(key))
            throw new TypeError(`unknown request property: ${key}`);
    if (input.version !== ARTIFACT_RECOMMENDATION_VERSION)
        throw new TypeError("version must be 1.0.0");
    if (typeof input.outcome !== "string" || input.outcome.trim().length === 0 || input.outcome.length > 4000)
        throw new TypeError("outcome must be a non-empty string of at most 4000 characters");
    if (OWN.call(input, "explicitType") && (typeof input.explicitType !== "string" || input.explicitType.trim().length === 0 || input.explicitType.length > 64))
        throw new TypeError("explicitType must be a non-empty string of at most 64 characters");
}
export function recommendArtifact(input) {
    assertRequest(input);
    ...
    const supported = requested !== undefined && ARTIFACT_TYPES.includes(requested);
    const unsafe = requested !== undefined && UNSAFE.test(outcome);
    let selected = inferred.type;
    let override = { disposition: "not-requested", reason: "No explicit artifact type was requested." };
    if (requested !== undefined && !supported)
        override = { requested: input.explicitType, disposition: "unsupported", reason: "The requested type is not one of skill, agent, hook, mcp, recipe, or plugin." };
    else if (requested !== undefined && unsafe)
        override = { requested, disposition: "unsafe", reason: "The requested override was not applied because the stated outcome seeks to bypass a safety or security control." };
    else if (requested !== undefined) {
        selected = requested;
        override = { requested, disposition: "respected", reason: "The explicit supported choice takes precedence over inferred routing." };
    }
