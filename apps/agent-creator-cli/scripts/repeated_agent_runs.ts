...
export type AgentRunProfile = "fast" | "standard" | "release";
export const AGENT_RUN_PROFILE_COUNTS: Readonly<Record<AgentRunProfile, number>> = Object.freeze({ fast: 1, standard: 3, release: 5 });
export type AgentVariant = "with_agent" | string;
export interface AgentPairSchedule { pair_index: number; seed: number; order: [AgentVariant, AgentVariant]; }
export interface PairEquivalence { model: string | null; tools: string[]; fixture_sha256: string; timeout_seconds: number; max_turns: number; }
export type PairedMetricName = "quality" | "latency_seconds" | "actual_turns" | "tokens" | "cost_usd";
export interface AgentRunEvidence { eval_id: string | number; pair_index: number; configuration: string; quality?: number | null; latency_seconds?: number | null; actual_turns?: number | null; tokens?: number | null; cost_usd?: number | null; }
export interface PairedMetricStatistics { direction: "candidate-minus-baseline"; expected_pairs: number; covered_pairs: number; missing_pairs: number; coverage: number; mean: number | null; p50: number | null; p95: number | null; stddev: number | null; confidence_interval_95: { lower: number; upper: number } | null; statistically_valid: boolean; }
...
export const evidenceSha256 = (value: unknown): string => createHash("sha256").update(canonical(value)).digest("hex");

export function parseAgentRunProfile(value: unknown): AgentRunProfile {
  const profile = String(value ?? "fast");
  if (!(profile in AGENT_RUN_PROFILE_COUNTS)) throw new TypeError("--run-profile must be fast, standard, or release");
  return profile as AgentRunProfile;
}

export function scheduleAgentPairs(binding: unknown, baselineConfiguration: string, profile: AgentRunProfile): AgentPairSchedule[] {
  if (!baselineConfiguration || baselineConfiguration === "with_agent") throw new TypeError("baseline configuration must be distinct from with_agent");
...
function percentile(values:number[],q:number):number|null { if(!values.length)return null;const sorted=[...values].sort((a,b)=>a-b),index=(sorted.length-1)*q,lower=Math.floor(index),upper=Math.ceil(index);return rounded(sorted[lower]+(sorted[upper]-sorted[lower])*(index-lower)); }

export function pairedMetricStatistics(deltas: Array<number | null>, expectedPairs = deltas.length): PairedMetricStatistics {
  if (!Number.isInteger(expectedPairs) || expectedPairs < 0 || deltas.length > expectedPairs) throw new TypeError("expected pair count must cover supplied evidence");
  const values = deltas.map(finite).filter((value): value is number => value !== null);
  const count = values.length;
  const mean = count ? values.reduce((sum, value) => sum + value, 0) / count : null;
  let stddev: number | null = null, confidence: { lower: number; upper: number } | null = null;
...
export function summarizePairedAgentRuns(current: AgentRunEvidence[], baseline: AgentRunEvidence[], expectedPairs: number) {
  const key = (run: AgentRunEvidence) => `${typeof run.eval_id}:${String(run.eval_id)}\u0000${run.pair_index}`;
  ...
  const metrics: PairedMetricName[] = ["quality", "latency_seconds", "actual_turns", "tokens", "cost_usd"];
  ...
  })) as Record<PairedMetricName, PairedMetricStatistics>;
