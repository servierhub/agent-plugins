 ...
 */
import { closeSync, constants, fstatSync, lstatSync, openSync, readFileSync, readSync, writeFileSync, existsSync, statSync, mkdirSync, opendirSync, } from "node:fs";
 ...
 */
function boundedFile(path, root, budget) {
    const ext = extname(path).toLowerCase();
...
function buildRun(root, runDir, budget) {
    let prompt = "";
    let evalId = null;
    ...
    if (isDir(outputsDir)) {
        ...
        for (const name of names.sort())
            outputFiles.push(boundedFile(join(outputsDir, name), root, budget));
        if (hasMore) {
            output_omission = {
                omitted_at_least: 1,
...
function findRunsRecursive(root, current, runs, budget, traversal, depth) {
    if (depth > REVIEW_OUTPUT_BUDGET.max_depth)
        return;
    if (runs.length >= REVIEW_OUTPUT_BUDGET.max_runs)
        return;
    if (traversal.directories >= REVIEW_OUTPUT_BUDGET.max_directories)
        return;
    if (!isDir(current))
        return;
    ...
    try {
        while (true) {
            ...
            if (!entry)
                break;
            traversal.entries++;
            if (traversal.entries > REVIEW_OUTPUT_BUDGET.max_directory_entries)
                break;
    ...
    for (const child of names.sort()) {
        ...
        if (runs.length >= REVIEW_OUTPUT_BUDGET.max_runs || traversal.entries > REVIEW_OUTPUT_BUDGET.max_directory_entries)
            break;
    }
}
...
function generateHtml(runs, agentName, previous, benchmark) {
...
async function main() {
    ...
    const agentName = values["agent-name"] || basename(workspace).replace("-workspace", "");
    const feedbackPath = join(workspace, "feedback.json");
    let previous = {};
