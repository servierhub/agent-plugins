...
export function parseAgent(path) {
    ...
    try {
        ...
        throw new AgentFormatError(`Cannot read agent file: ${error.message}`);
    }
    ...
    try {
        ...
        throw new AgentFormatError(`Invalid YAML frontmatter: ${error.message}`);
    }
    ...
    if (meta.description !== undefined && meta.description !== null) {
        if (typeof meta.description !== "string" || !meta.description.trim()) {
            throw new AgentFormatError("Frontmatter field 'description' must be a non-empty string when present");
        }
    ...
    if (meta.model !== undefined && meta.model !== null) {
        if (typeof meta.model !== "string" || !meta.model.trim()) {
            throw new AgentFormatError("Frontmatter field 'model' must be a non-empty string when present");
        }
...
export function renderAgent(name, description, model, body) {
    const metadata = { name };
    if (description)
