...
export function verifyPlugin(options: VerifyOptions) {
    ...
    let review: Status = "na", reviewReason = "not required", approvalEvidence:string[]=[];
    const workspace = options.integration ? resolve(options.integration) : "";
    if (strict) {
        if (!workspace || !existsSync(join(workspace, "review.html"))) { review = "blocked"; reviewReason = "integration review.html missing"; }
        else if (parsed.profile !== "production") { review=parsed.humanReview==="pass"?"pass":"blocked";reviewReason=review==="pass"?"human review complete":"human review "+parsed.humanReview; }
        else if (!options.approval||!options.approvalTrustPolicy||!options.testEvidence||!archive) { review="blocked";reviewReason="production requires approval file, trust policy, test evidence, and exact bindings; --human-review cannot approve release"; }
        else try {const expected=productionBindings(root,archive,workspace,options.testEvidence),verified=verifyProductionApproval(options.approval,options.approvalTrustPolicy,expected);review="pass";reviewReason="identity-bound production approval verified";approvalEvidence=["request "+verified.request_id,"approval "+verified.approval_sha256,...Object.entries(expected).map(([k,v])=>k+": "+v)];}catch(error){review="blocked";reviewReason=(error as Error).message;}
    }
...
if (!import.meta.url.includes("/$bunfs/") && process.argv[1] && resolve(fileURLToPath(import.meta.url)) === resolve(process.argv[1])) main();
