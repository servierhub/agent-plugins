...
export type PackageProfile = "authoring" | "release";
...
export function shouldExcludePackagePath(path: string, rootArg: string, profile: PackageProfile = "authoring"): boolean {
  const root = resolve(rootArg);
  const rel = relative(root, resolve(path));
  if (!rel || rel === ".." || rel.startsWith(".." + sep)) return false;
  const parts = rel.split(sep);
  if (ROOT_EXCLUDED_PARTS.has(parts[0]) || (parts[0].startsWith(".") && !ROOT_INCLUDED_HIDDEN_PARTS.has(parts[0]))) return true;
  if (parts.some((part, index) => EXCLUDED_PARTS.has(part) && !(part === "node_modules" && parts[index - 1] === "vendor"))) return true;
  const suffix = parts.at(-1)?.match(/\.[^.]+$/)?.[0] ?? "";
  if (EXCLUDED_SUFFIXES.has(suffix)) return true;
  if (profile !== "release") return false;
  ...
  if (parts.some((part, index) => part === "node_modules" && parts[index - 1] === "vendor")) return true;
  if (/^tsconfig(?:\.[^.]+)?\.json$/.test(parts.at(-1) ?? "")) return true;
  if (parts[0] === "skills" && parts.length >= 3) {
    if (parts[2] === "scripts") return true;
    if (suffix === ".ts" && !parts.at(-1)?.endsWith(".d.ts")) return true;
  }
  if (parts[0] === "tests" || parts[0] === "scripts") return true;
  if (parts[0] === "contracts" && parts[1] === "capability-contract" && ["src", "tests", "scripts"].includes(parts[2] ?? "")) return true;
  return false;
}

function containmentError(path: string, root: string, result: PathContainmentResult, expected: ExpectedPathKind): Error {
  const rel = relative(root, path) || ".";
  if (!result.contained) return new Error(`package path escapes plugin root (${result.status}): ${rel}`);
  if (result.kindOutcome === "missing") return new Error(`package path is missing: ${rel}`);
  return new Error(`package path has wrong kind (expected ${expected}, found ${result.kind}): ${rel}`);
}

function requireContained(root: string, path: string, expected: ExpectedPathKind): PathContainmentResult {
  const result = resolveContainedPath(root, path, { expectedKind: expected });
  if (!result.contained || result.kindOutcome !== "match") throw containmentError(path, root, result, expected);
  return result;
}

export function collectPackageFiles(rootArg: string, omitted: string[] = [], profile: PackageProfile = "authoring"): PackageFile[] {
  const lexicalRoot = resolve(rootArg);
  const rootCheck = resolveContainedPath(lexicalRoot, lexicalRoot, { expectedKind: "directory" });
  if (!rootCheck.contained || rootCheck.kindOutcome !== "match") throw containmentError(lexicalRoot, lexicalRoot, rootCheck, "directory");
  ...
  function walk(directory: string): void {
    ...
    if (visited.has(resolvedDirectory)) throw new Error("package directory cycle detected: " + relative(root, directory));
    ...
    for (const name of readdirSync(resolvedDirectory).sort()) {
      ...
      if (shouldExcludePackagePath(path, root, profile) || omittedSet.has(path)) continue;
      const rel = relative(root, path);
      if (!rel || rel === ".." || rel.startsWith(".." + sep)) throw new Error("package path escapes plugin root: " + path);
      ...
      if (stat.isSymbolicLink()) throw new Error("symbolic links are not allowed in packages: " + rel);
      if (!containment.contained || containment.kindOutcome !== "match") throw containmentError(path, root, containment, "any");
