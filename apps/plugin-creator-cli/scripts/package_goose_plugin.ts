...
import { collectPackageFiles, type PackageProfile } from "./package_manifest.js";
...
function validateOfflineSkills(root: string, profile: PackageProfile): string[] {
  ...
  for (const skill of readdirSync(skillsRoot).sort()) {
    ...
    if (profile === "release") {
      ...
      if (!nativeNames.some((name) => existsSync(join(root, "bin", name)))) missing.push(`bin/${nativeNames[0]}`);
      continue;
    }
...
export function main(): void {
  ...
  if (profileValue !== "authoring" && profileValue !== "release") fail("--profile must be authoring or release");
  const profile=profileValue as PackageProfile;
  if (!pluginDirArg) { console.error("usage: package_goose_plugin.js <plugin_dir> [output.zip] [--profile authoring|release] [--production --approval FILE --approval-trust-policy FILE --integration DIR --test-evidence FILE]"); process.exit(2); }
  ...
  let files;
  try { files = collectPackageFiles(root, [output], profile); } catch (error) { fail((error as Error).message); }
  const manifest = JSON.parse(readFileSync(join(root, "plugin.json"), "utf8"));
  const zip = new AdmZip();
  for (const file of files) zip.addFile(manifest.name + "/" + file.relative, readFileSync(file.absolute), "", file.mode);
