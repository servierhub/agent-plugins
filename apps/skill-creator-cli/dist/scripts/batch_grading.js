...
import { buildGradingJudgment, validateGradingRequest } from "./delegated_grading_contracts.js";
 ...
 */
export function groupPendingBatches(workspaceArg) {
    ...
    for (const fileName of fileNames) {
        ...
        if (existsSync(judgmentPath))
            continue; // already judged, nothing to batch
        const manifestEntry = manifest[invocationId];
        if (!manifestEntry) {
            errors.push({ invocation_id: invocationId, message: "No prepare-grading manifest entry for this invocation_id" });
            continue;
        }
        let requestRaw;
        try {
            ...
            errors.push({ invocation_id: invocationId, message: `Request file is unreadable or escapes containment: ${error.message}` });
            continue;
        }
        if (!requestRaw) {
            errors.push({ invocation_id: invocationId, message: "Request file is missing or not valid JSON" });
            continue;
        }
        let request;
        try {
            ...
            errors.push({ invocation_id: invocationId, message: `Request schema is invalid: ${error.message}` });
            continue;
        }
 ...
 */
export function applyBatchResults(workspaceArg, batchId, results) {
    const workspace = resolve(workspaceArg);
    const batchDir = join(workspace, BATCH_DIR_NAME);
    const judgmentDir = join(workspace, JUDGMENT_DIR_NAME);
    const errors = [];
    ...
    const batch = loadJson(batchDir, batchPath, batchId + ".json");
    if (!batch) {
        errors.push({ invocation_id: "*", message: `Batch ${batchId} is missing or unreadable` });
        return { schema_version: "1.0", workspace, batch_id: batchId, judgments_written: 0, errors };
    }
    const requestDir = join(workspace, REQUEST_DIR_NAME);
    const requestsById = new Map(batch.requests.map((r) => [r.invocation_id, r]));
    if (!Array.isArray(results)) {
        errors.push({ invocation_id: "*", message: "Batch results must be a JSON array" });
        return { schema_version: "1.0", workspace, batch_id: batchId, judgments_written: 0, errors };
    }
    for (const raw of results) {
        const entry = raw;
        const invocationId = String(entry?.invocation_id ?? "");
        if (!requestsById.has(invocationId)) {
            errors.push({ invocation_id: invocationId || "(missing)", message: `invocation_id is not part of batch ${batchId}` });
            continue;
        }
        if (!entry || !["pass", "fail", "inconclusive"].includes(String(entry.verdict))) {
            errors.push({ invocation_id: invocationId, message: "verdict must be pass, fail, or inconclusive" });
            continue;
        }
        const requestPath = join(requestDir, invocationId + ".json");
        const request = loadJson(requestDir, requestPath, invocationId + ".json");
        if (!request) {
            errors.push({ invocation_id: invocationId, message: "Originating request file is missing or unreadable" });
            continue;
        }
        try {
...
export async function mainGroup(argv = process.argv.slice(2)) {
    const workspace = argv[0];
    if (!workspace) {
        console.error("usage: group-grading-batches <workspace>");
        return 2;
    }
    try {
        ...
        console.log(JSON.stringify({ schema_version: "1.0", command: "group-grading-batches", status: "failure", reason: error.message }, null, 2));
        return 1;
    }
}
/** CLI entry for `skill-creator apply-grading-batch <workspace> <batch-id> <results.json>`. */
export async function mainApply(argv = process.argv.slice(2)) {
    const [workspace, batchId, resultsPath] = argv;
    if (!workspace || !batchId || !resultsPath) {
        console.error("usage: apply-grading-batch <workspace> <batch-id> <results.json>");
        return 2;
    }
    try {
        ...
        try {
            ...
            throw new Error(`Results file ${resultsPath} is missing or not valid JSON: ${error.message}`);
        }
        console.log(JSON.stringify(applyBatchResults(workspace, batchId, raw), null, 2));
        return 0;
    }
    catch (error) {
        console.log(JSON.stringify({ schema_version: "1.0", command: "apply-grading-batch", status: "failure", reason: error.message }, null, 2));
        return 1;
    }
}
