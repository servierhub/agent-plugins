...
function semanticEvidence(runDir:string):any[]{const root=join(runDir,"grader-evidence");if(!isDirectory(root))return[];return readdirSync(root).filter(name=>name.endsWith(".json")).sort().map(name=>loadJson(join(root,name))).filter(Boolean);}
function same(a:unknown,b:unknown):boolean{return gradingCanonical(a)===gradingCanonical(b);}
function verifyEvaluatorGrade(runDir:string,label:string,metadata:any,manifest:any,grading:any,errors:string[]):void{
  ...
  for(const assertion of assertions){const ah=assertionHash(assertion), common={id:assertion.id,version:assertion.version,classification:assertion.classification,text:assertion.criterion,criterion:assertion.criterion,assertion_sha256:ah,variant_sha256:variantHash,output_sha256:outputHash};
    if(assertion.classification==="deterministic"){const checked=deterministicCheck(assertion,output);derived.push({...common,verdict:checked.verdict,passed:checked.verdict==="pass",evidence:checked.evidence});continue;}
    const files=semanticEvidence(runDir).filter(j=>j.assertion_sha256===ah), valid:GraderJudgment[]=[];
  ...
  for(let i=0;i<derived.length;i++){const a=claimed[i],d=derived[i];for(const key of ["id","version","classification","assertion_sha256","variant_sha256","output_sha256","verdict","passed"])if(!a||!same(a[key],d[key]))errors.push(label+" expectation "+(d.id??i)+" "+key+" is not independently derived");}
...
export function validateExecutionEvidence(
  ...
  for (const runDir of planned) {
    ...
    const fields: string[] = ["skill_source_sha256", "eval_plan_sha256", "scenario_sha256", "evidence_mode_sha256", "grading_plan_sha256", "configuration", "run_index", "pair_index", "seed", "order", "order_position"];
    ...
    const grading=loadJson(join(runDir,"grading.json")), mode=metadataMode(runDir), executor=(manifest as any).executor;
    ...
    if(mode.planned==="goose-evaluator"&&executor?.adapter!=="goose") errors.push(`${label} planned Goose execution cannot be downgraded by deleting or changing its executor marker`);
    if(mode.planned==="manual-governed-import") verifyManualImport(runDir,label,metadata,manifest,errors);
    if(mode.planned==="goose-evaluator"){
      ...
      else verifyEvaluatorGrade(runDir,label,metadata,manifest,grading,errors);
    }
