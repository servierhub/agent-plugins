...
export function verifySkill(options: VerifySkillOptions) {
  ...
  if (strict) {
    if (!benchmark) {
      ...
      if(!validN||paired?.statistically_valid!==true||!validCi){behavior="fail";behaviorReason="release requires finite integer paired sample count >= 2, statistically_valid true, and a finite ordered 95% confidence interval";}
      else {behavior = summary.current >= minimum && delta! >= minDelta ? "pass" : "fail";behaviorReason=`pass rate ${summary.current.toFixed(4)} (minimum ${minimum}); delta ${delta!.toFixed(4)} (minimum ${minDelta})`; }
    } else {
...
if (!import.meta.url.includes("/$bunfs/") && invokedPath && fileURLToPath(import.meta.url) === invokedPath) main();
