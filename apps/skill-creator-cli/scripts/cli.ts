...
async function embedded(command: Command, args: string[]): Promise<EmbeddedResult> {
  switch (command) {
...
if ((import.meta as ImportMeta & { main?: boolean }).main || !import.meta.url.includes("/$bunfs/")) process.exitCode = await runCli();