...
const known=new Set(["--source","--destination","--dry-run","--force","--help","-h"]);for(let i=0;i<args.length;i++){const key=args[i].split("=")[0];if(!known.has(key))fail("unknown option: "+args[i]);if((key==="--source"||key==="--destination")&&!args[i].includes("="))i++;}
...
const result={status:dryRun?"dry-run":"installed",source,destination,releaseKey,files:actual.length+1,replacing:Boolean(destinationStat)};if(dryRun){console.log(JSON.stringify(result));process.exit(0);}if(destinationStat)rmSync(destination,{recursive:true,force:true});mkdirSync(path.dirname(destination),{recursive:true});cpSync(source,destination,{recursive:true,errorOnExist:true,force:false,dereference:false});console.log(JSON.stringify(result));
