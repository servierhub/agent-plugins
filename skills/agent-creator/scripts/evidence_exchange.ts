...
export const TRUST_LEVELS = ["unverified", "human-reviewed", "independently-verified"] as const;
...
export interface EvidenceJob {
  ...
  job_id: string;
  created_at: string;
  agent: { name: string; ref: string; sha256: string };
  eval_set: { ref: string; sha256: string };
  fixture_root: string;
  fixtures: Array<{ source: string; ref: string; sha256: string }>;
...
export interface EvidenceRun {
  ...
  response: { text: string; sha256: string };
  timing: { total_duration_seconds: number; total_tokens?: number };
  provenance: {
  ...
  trust: { level: TrustLevel; reason: string; reviewed_by?: string };
}
...
function readJson(path: string): any {
  ...
  catch (error) { throw new EvidenceDiagnostic("INVALID_JSON", `${path}: ${(error as Error).message}`); }
}
...
function isRfc3339(value: string): boolean {
  ...
  if (month < 1 || month > 12 || parts[3] > 23 || parts[4] > 59 || parts[5] > 59) return false;
...
function matchesSchema(value: unknown, schema: Schema, path = "$" ): string[] {
  const errors: string[] = [];
  const types = Array.isArray(schema.type) ? schema.type : schema.type ? [schema.type] : [];
  const isType = (type: string) => type === "object" ? object(value) : type === "array" ? Array.isArray(value) : type === "integer" ? Number.isInteger(value) : type === "number" ? typeof value === "number" && Number.isFinite(value) : typeof value === type;
  if (types.length && !types.some(isType)) return [path + " has an invalid type"];
  if (schema.const !== undefined && value !== schema.const) errors.push(path + " must equal the schema constant");
  if (schema.enum && !schema.enum.includes(value)) errors.push(path + " is not an allowed value");
  if (schema.anyOf && !schema.anyOf.some((candidate: Schema) => matchesSchema(value, candidate, path).length === 0)) errors.push(path + " does not match any allowed schema");
  if (typeof value === "string") {
    ...
    if (schema.format === "date-time" && !isRfc3339(value)) errors.push(path + " is not a strict RFC3339 timestamp");
  }
  ...
  if (object(value)) {
    ...
    for (const [key, child] of Object.entries(properties)) if (key in value) errors.push(...matchesSchema(value[key], child as Schema, path + "." + key));
  }
  for (const condition of schema.allOf ?? []) if ((!condition.if || matchesSchema(value, condition.if, path).length === 0) && condition.then) errors.push(...matchesSchema(value, condition.then, path));
  return errors;
}
...
function validateEvalId(id: unknown, label = "Eval id"): asserts id is string | number {
  if ((typeof id === "string" && id.length > 0 && id.length <= 256 && !/[\u0000-\u001F\u007F]/.test(id) && hasOnlyUnicodeScalars(id)) || Number.isInteger(id)) return;
  throw new EvidenceDiagnostic("INVALID_EVAL_ID", label + " must be an integer or 1-256 printable Unicode scalar characters");
}
function evalDirectoryName(id: string | number): string {
...
function parseAgentName(text: string): string {
  const match = /^---\s*\n[\s\S]*?^name:\s*["']?([^"'\n]+)["']?\s*$[\s\S]*?^---\s*$/m.exec(text);
  if (!match?.[1]?.trim()) throw new EvidenceDiagnostic("INVALID_AGENT", "Agent frontmatter must contain name");
  return match[1].trim();
}

function metadata(evalCase: any): Omit<EvalRecord, "metadata_sha256"> {
...
function validateEvalCases(document: any): any[] {
  ...
  for (const item of document.evals) {
    if (!object(item)) throw new EvidenceDiagnostic("INVALID_EVAL_SET", "Each eval must be an object");
    validateEvalId(item.id);
    if (ids.has(String(item.id))) throw new EvidenceDiagnostic("INVALID_EVAL_SET", `Duplicate eval id: ${item.id}`);
...
export function exportEvidenceJob(options: {
  agentPath: string; evalSetPath: string; outputDir: string; fixtureRoot?: string;
  configurations?: string[]; now?: string;
}): EvidenceJob {
  const agentPath = resolve(options.agentPath), evalSetPath = resolve(options.evalSetPath);
  const outputDir = resolve(options.outputDir);
  if (existsSync(outputDir)) throw new EvidenceDiagnostic("OUTPUT_EXISTS", `Refusing to overwrite ${outputDir}`);
  ...
  const fixtureMap = new Map<string, { source: string; ref: string; sha256: string }>();
  for (const evalCase of evalCases) for (const raw of evalCase.files ?? []) {
  ...
  const evals: EvalRecord[] = evalCases.map(item => {
    const base = metadata(item);
    return { ...base, metadata_sha256: sha256(canonicalJson(base as unknown as Json)) };
  });
  const identity = {
  ...
  const job: EvidenceJob = {
    ...
    job_id: `job_${sha256(canonicalJson(identity as unknown as Json))}`,
...
function validateJob(job: any, jobPath: string): asserts job is EvidenceJob {
  ...
  if (!Array.isArray(job.fixtures) || !job.configurations.length || !job.configurations.every((value: unknown) => typeof value === "string" && /^[A-Za-z0-9._-]+$/.test(value)) || new Set(job.configurations).size !== job.configurations.length)
  ...
  for (const evalCase of job.evals) {
    if (!object(evalCase)) throw new EvidenceDiagnostic("INVALID_JOB", "Job evals must be objects");
    validateEvalId(evalCase.id, "Job eval id");
    if (ids.has(String(evalCase.id))) throw new EvidenceDiagnostic("INVALID_JOB", "Duplicate job eval id: " + evalCase.id);
  ...
  for (let index = 0; index < sourceEvals.length; index++) {
    ...
    const expected = { ...canonical, metadata_sha256: sha256(canonicalJson(canonical as unknown as Json)) };
  ...
  const identity = { agent_sha256: job.agent.sha256, eval_set_sha256: job.eval_set.sha256, fixtures: [...job.fixtures].sort((a: any, b: any) => String(a.source).localeCompare(String(b.source))), configurations: job.configurations, evals: job.evals };
  const expectedJobId = "job_" + sha256(canonicalJson(identity as unknown as Json));
  if (job.job_id !== expectedJobId) throw new EvidenceDiagnostic("INVALID_JOB", "job_id does not match canonical manifest content; expected " + expectedJobId);
}

export function validateEvidenceRun(document: any, job: EvidenceJob): EvidenceRun {
  const schemaErrors = matchesSchema(document, RUN_JSON_SCHEMA as unknown as Schema);
  ...
  if (document.job_id !== job.job_id) throw new EvidenceDiagnostic("WRONG_JOB", "Run targets " + document.job_id + "; expected " + job.job_id);
  if (!job.evals.find(item => item.id === document.eval_id)) throw new EvidenceDiagnostic("WRONG_JOB", "Eval " + document.eval_id + " is not an exact id/type match in job " + job.job_id);
  if (!job.configurations.includes(document.configuration)) throw new EvidenceDiagnostic("WRONG_JOB", "Configuration " + document.configuration + " is not in job " + job.job_id);
  if (document.response.sha256 !== sha256(document.response.text)) throw new EvidenceDiagnostic("INVALID_RUN", "response.sha256 must match response.text exactly");
  return document as EvidenceRun;
}

export function importEvidenceRun(options: { jobPath: string; runPath: string; workspace: string }): {
  status: "imported" | "duplicate"; run_id: string; destination: string; evidence_sha256: string;
} {
...
export function exampleRun(job: EvidenceJob): EvidenceRun {
...
function main(): void {
  ...
  if (command === "export") {
    ...
    if (!values.agent || !values["eval-set"] || !values.output) usage();
    ...
    if (!values.job || !values.run || !values.workspace) usage();
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try { main(); }
    ...
    const diagnostic = error instanceof EvidenceDiagnostic ? error : new EvidenceDiagnostic("IMPORT_FAILED", (error as Error).message);
