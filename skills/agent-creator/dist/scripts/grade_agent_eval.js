...
function verifiedRuns(evalName, evalDir, meta) {
    ...
    if (!Array.isArray(names) || names.length < 2 || !names.every((name) => typeof name === "string" && name.length > 0) || new Set(names).size !== names.length)
        throw new Error(evalName + " requires at least two unique declared variants; rerun both variants");
    if (!meta.variant_sources || typeof meta.variant_sources !== "object" || Array.isArray(meta.variant_sources))
        throw new Error(evalName + " lacks immutable variant_sources; rerun both variants");
    ...
    if (JSON.stringify(manifestNames) !== JSON.stringify(declared) || manifestNames.some(name => !manifest[name] || typeof manifest[name].source_sha256 !== "string" || !/^[a-f0-9]{64}$/.test(manifest[name].source_sha256)))
        throw new Error(evalName + " has an invalid variant source manifest; rerun both variants");
    ...
    if (typeof meta.assertion_hash !== "string" || meta.assertion_hash !== expected)
        throw new Error("Canonical assertion/variant hash changed for " + evalName + "; rerun both variants");
    const available = declared.filter(name => isDir(join(evalDir, name)));
    if (!available.length)
        throw new Error(evalName + " has no available variant evidence");
    return available.map(name => { const dir = join(evalDir, name), marker = join(dir, "assertion_hash.txt"); if (!isDir(join(dir, "outputs")) || !existsSync(marker) || readFileSync(marker, "utf-8").trim() !== expected)
        throw new Error("Assertion/variant hash changed for " + evalName + "/" + name + "; rerun both variants"); const grading = join(dir, "grading.json"); if (existsSync(grading) && JSON.parse(readFileSync(grading, "utf-8")).assertion_hash !== expected)
        throw new Error("Assertion/variant hash changed for " + evalName + "/" + name + "; rerun both variants"); return { name, dir }; });
}
async function main() { const { positionals, values } = parseArgs({ args: process.argv.slice(2), allowPositionals: true, options: { "llm-grader": { type: "boolean", default: false }, "goose-cli": { type: "string", default: process.env.AGENT_CREATOR_GOOSE_CLI ?? "goose" }, grader: { type: "string", multiple: true }, model: { type: "string" }, "max-grader-calls": { type: "string", default: "100" } } }); const [arg] = positionals; if (!arg) {
    ...
    throw new Error("--max-grader-calls must be a non-negative integer"); const budget = { used: 0, max }, workspace = resolve(arg), command = values["goose-cli"].split(/\s+/).filter(Boolean); let n = 0; for (const en of readdirSync(workspace).filter(x => x.startsWith("eval-")).sort()) {
    const ed = join(workspace, en), mp = join(ed, "eval_metadata.json");
    if (!existsSync(mp))
        continue;
    const meta = JSON.parse(readFileSync(mp, "utf-8"));
    const runs = verifiedRuns(en, ed, meta);
    for (const run of runs) {
        await gradeRun(run.dir, run.name, meta, graders, command, values["llm-grader"], budget);
        n++;
    }
