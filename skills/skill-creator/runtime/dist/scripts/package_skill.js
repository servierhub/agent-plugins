...
function executableNames(skillName) {
    return process.platform === "win32" ? [skillName + ".exe", skillName] : [skillName, skillName + ".exe"];
}
function findNativeExecutable(skillPath, skillName, explicit) {
    ...
    if (!found)
        return null;
    if (explicit && ![skillName, skillName + ".exe"].includes(basename(found)))
        return null;
    return found;
}
...
function shouldExclude(relParts, profile) {
    if (relParts.some((part, index) => EXCLUDE_DIRS.has(part) && !(profile === "authoring" && part === "node_modules" && relParts[index - 1] === "vendor")))
        return true;
    if (relParts.length > 1 && ROOT_EXCLUDE_DIRS.has(relParts[1]))
        return true;
    const name = relParts[relParts.length - 1];
    if (EXCLUDE_FILES.has(name) || EXCLUDE_GLOBS.some((pat) => matchGlob(name, pat)))
        return true;
    if (profile === "release") {
        const skillParts = relParts.slice(1);
        if (["scripts", "tests", "eval-viewer"].includes(skillParts[0] ?? ""))
            return true;
...
function validateOfflineBundle(skillPath, profile, nativeExecutable) {
    ...
    if (profile === "release") {
        if (!nativeExecutable)
            missing.push(`bin/${basename(skillPath)} (native executable)`);
        return missing;
    }
...
function collectFiles(current, out, parentDir, profile) {
    for (const entry of readdirSync(current).sort()) {
        const full = join(current, entry);
        const relPath = full.slice(parentDir.length + 1);
        if (shouldExclude(relPath.split(/[\\/]/), profile))
            continue;
        if (isSymlink(full))
            throw new Error(`Refusing to package symbolic link: ${relPath}`);
        if (isDir(full))
            collectFiles(full, out, parentDir, profile);
        else
            out.push(full);
    }
}
export function packageSkill(skillPathArg, outputDirArg, profile = "authoring", executableArg) {
    ...
    try {
        ...
        for (const filePath of allFiles) {
            ...
            if (shouldExclude(relParts, profile))
                continue;
