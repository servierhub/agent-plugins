...
test("every unified subcommand exposes help", () => {
  for (const command of ["validate", "audit", "design-evals", "freeze-evals", "analyze", "full-eval", "trigger-eval", "aggregate", "review", "verify", "package"]) assert.equal(invoke([command, "--help"]).status, 0, command);
});
