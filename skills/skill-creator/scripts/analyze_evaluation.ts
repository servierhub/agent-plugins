...
function main(): void { try { const {positionals,values}=parseArgs({args:process.argv.slice(2),allowPositionals:true,options:{"skill-path":{type:"string"},output:{type:"string",short:"o"}}}); if(!positionals[0]||!values["skill-path"])throw new TypeError("usage: analyze_evaluation.js <workspace> --skill-path <dir> [-o analysis.json]"); const result=analyzeEvaluation(positionals[0],values["skill-path"]),text=JSON.stringify(result,null,2)+"\n"; if(values.output){mkdirSync(dirname(resolve(values.output)),{recursive:true});writeFileSync(resolve(values.output),text);} console.log(text.trim()); process.exit(result.status==="blocked"?3:0); } catch(error){console.error(`analyze_evaluation: ${(error as Error).message}`);process.exit(2);} }
