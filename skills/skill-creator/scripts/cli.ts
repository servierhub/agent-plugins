...
type Command = "validate" | "audit" | "design-evals" | "analyze" | "trigger-eval" | "aggregate" | "review" | "verify" | "package" | "full-eval";
...
const commands: Record<Command, { entry: string; usage: string; required: (args: string[]) => boolean }> = {
  ...
  "full-eval": { entry: "full_eval.js", usage: "full-eval <skill-directory> [--workspace <dir>] [--eval-set <file>] [--dry-run] [--resume|--retry|--cancel] [options]", required: (a) => Boolean(a[0] && !a[0].startsWith("-")) },
