...
import { artifactHash, expectedExecutionBinding, listRunDirs } from "./evaluation_provenance.js";
...
export async function executePairedRuns(options) { const runner = createRunner(options.runner); if (typeof runner.executePaired !== "function")
    return { status: "blocked", completed: [], failures: [{ run: "*", code: "unsupported-runner", message: "Runner does not support paired execution", exit_reason: "unsupported-runner" }] }; const completed = [], failures = []; const skillName = basename(resolve(options.skillPath)); for (const runDir of listRunDirs(resolve(options.workspace))) {
    ...
    try {
        ...
        plan = { prompt: String(metadata.prompt ?? ""), assertions: Array.isArray(metadata.assertions) ? metadata.assertions.map(String) : [], cwd, model: options.model ?? (metadata.model ? String(metadata.model) : null), tools, capabilities: declared, maxTurns: Number(budget.max_turns ?? 40), timeoutSeconds: Number(budget.timeout_seconds ?? 300), configuration, skillName, signal: options.signal };
        writeEvidence(runDir, await runner.executePaired(plan), plan, source);
...
} const cancelled = failures.some(x => x.code === "cancelled"), blocked = failures.some(x => ["command-not-found", "unsupported-runner", "model-unavailable", "tool-unavailable", "capability-mismatch"].includes(x.code)); return { status: cancelled ? "cancelled" : failures.length ? (blocked ? "blocked" : "failed") : "complete", completed, failures }; }
