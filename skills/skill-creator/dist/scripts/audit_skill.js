...
function directory(path) { try {
    return statSync(path).isDirectory();
}
catch {
    return false;
} }
function finding(findings, severity, rule, message, path) { findings.push({ severity, rule, message, ...(path ? { path } : {}) }); }
function bodyAfterFrontmatter(content) { return content.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/, ""); }
function localMarkdownLinks(content) { return [...content.matchAll(/\[[^\]]*\]\(([^)]+)\)/g)].map(m => m[1].trim().split("#")[0]).filter(Boolean).filter(p => !/^(?:https?:|mailto:|#)/i.test(p)); }
function wordCount(value) { return value.trim().split(/\s+/).filter(Boolean).length; }
function hasNonEnglishSignals(value) {
    return /\b(?:utilisez|utiliser|quand|fichiers?|competence|habilidad|cuando|archivos?|verwenden|wenn|dateien?)\b/i.test(value);
}
export function auditSkill(skillArg) {
    const skill = resolve(skillArg), findings = [], skillMd = join(skill, "SKILL.md");
    const [valid, validationMessage] = validateSkill(skill);
    if (!valid)
        finding(findings, "error", "conformance", validationMessage, "SKILL.md");
    ...
    try {
        ...
        finding(findings, "error", "parse", error.message, "SKILL.md");
    }
    const lines = content ? content.split(/\r?\n/) : [];
    const body = bodyAfterFrontmatter(content);
    if (description) {
        if (/[^\x00-\x7F]/.test(description) || hasNonEnglishSignals(description))
            finding(findings, "error", "metadata-language", "Description must use English-only portable metadata; non-English text was detected.", "SKILL.md");
        if (description.length > 350)
            finding(findings, "warning", "description-concision", `Description has ${description.length} characters; prefer one or two concise discovery sentences.`, "SKILL.md");
        if (!/\bUse (?:when|for)\b/.test(description))
            finding(findings, "warning", "description-activation", "Description should state when the Skill activates, typically with a concise 'Use when' or 'Use for' clause.", "SKILL.md");
        if (/^(?:I|You|We|Use\b)/i.test(description))
            finding(findings, "error", "description-person", "Description must begin with a third-person statement of capability, not first/second person or an imperative.", "SKILL.md");
    }
    if (hasNonEnglishSignals(body))
        finding(findings, "error", "instruction-language", "SKILL.md instructions must be written in English; non-English instructional text was detected.", "SKILL.md");
    if (lines.length > 500)
        finding(findings, "error", "entrypoint-line-budget", `SKILL.md has ${lines.length} lines; keep it at or below 500 and move conditional or deep detail into directly linked references.`, "SKILL.md");
    else if (lines.length > 450)
        finding(findings, "warning", "entrypoint-line-budget", `SKILL.md has ${lines.length} lines and is approaching the 500-line limit.`, "SKILL.md");
    if (/[A-Za-z0-9_.-]\\[A-Za-z0-9_.-]/.test(body))
        finding(findings, "error", "portable-paths", "Use forward slashes in Skill paths; Windows-style backslash paths are not portable.", "SKILL.md");
    const links = localMarkdownLinks(content);
    for (const target of links) {
        if (isAbsolute(target) || target.startsWith("../"))
            finding(findings, "error", "self-contained-reference", `Reference must stay inside the Skill directory: ${target}`, target);
        else if (!existsSync(join(skill, target)))
            finding(findings, "error", "missing-reference", `Linked resource does not exist: ${target}`, target);
        else if (target.endsWith(".md")) {
            const reference = readFileSync(join(skill, target), "utf8"), referenceLines = reference.split(/\r?\n/).length;
            if (hasNonEnglishSignals(reference))
                finding(findings, "error", "instruction-language", `Linked Skill instructions must be written in English: ${target}`, target);
            if (referenceLines > 300 && !/^## Contents$/m.test(reference))
                finding(findings, "warning", "reference-table-of-contents", `${target} has ${referenceLines} lines and should include a Contents section.`, target);
            for (const nested of localMarkdownLinks(reference))
                if (nested.endsWith(".md"))
                    finding(findings, "warning", "deep-reference-chain", `${target} links to ${nested}; prefer linking required references directly from SKILL.md.`, target);
        }
    }
...
function main() {
    try {
        ...
        if (!positionals[0] || !directory(resolve(positionals[0])))
            throw new TypeError("usage: audit_skill.js <skill-directory> [-o audit.json]");
        ...
        console.error(`audit_skill: ${error.message}`);
        process.exit(2);
    }
}
