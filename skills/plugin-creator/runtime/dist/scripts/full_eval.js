...
import { isNativeRuntime, resolveRelatedCommand } from "./related-command-runtime.js";
...
function runCancellableChild(command, args, deadline, cancelPath, onTelemetry, shouldStop) { return new Promise((resolveChild, reject) => { const child = spawn(command, args, { detached: process.platform !== "win32", stdio: ["ignore", "pipe", "pipe"] }); let stdout = "", stderr = "", settled = false, cancelled = false, timedOut = false, forceTimer = null; child.stdout?.on("data", (data) => stdout += data.toString()); child.stderr?.on("data", (data) => stderr += data.toString()); const finish = (status) => { if (settled)
    ...
    stop("deadline"); }); }
async function packagePlugin(root, archive, deadline, cancelPath, onTelemetry, shouldStop) { if (import.meta.url.includes("/$bunfs/")) {
    if (readCancellation(cancelPath) || shouldStop?.())
        return { status: null, stdout: "", stderr: "", cancelled: true, timedOut: false };
    if (Date.now() >= deadline)
        return { status: null, stdout: "", stderr: "", cancelled: false, timedOut: true };
    const result = await runEmbedded((await import("./package_goose_plugin.js")).main, [root, archive]);
    return { status: result.status, stdout: result.stdout, stderr: result.stderr, cancelled: false, timedOut: false };
} return runCancellableChild(process.execPath, [join(HERE, "package_goose_plugin.js"), root, archive], deadline, cancelPath, onTelemetry, shouldStop); }
