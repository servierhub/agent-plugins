...
export async function evaluate(args) {
    ...
    if (missing.length) {
        return { status: 3, output: { policy_error: "behavioral instructions do not match executable policy", missing_instructions: missing, input_fingerprint: hash(JSON.stringify(input)).slice(0, 12) } };
    }
    const output = await policy.evaluate(input, scenario);
    return { status: 0, output: { ...output, input_fingerprint: hash(JSON.stringify(input)).slice(0, 12), artifact_sha256: hash(artifact.text), policy_sha256: hash(readFileSync(modulePath)) } };
}
...
export async function runGoldenBehavior(mode, args) { if (mode === "evaluate")
    return evaluate(args); if (mode === "challenge")
    return challenge(args); throw Error("usage: golden_behavior_runner <evaluate|challenge> ..."); }
if (!import.meta.url.includes("/$bunfs/") && import.meta.main) {
    const [mode, ...args] = process.argv.slice(2);
    try {
        const result = await runGoldenBehavior(mode, args);
