...
import { EMBEDDED_GOLDEN } from "./embedded_resources.js";
import { runGoldenBehavior } from "./golden_behavior_runner.js";
import { budgetSnapshot, normalizeRunTelemetry } from "./usage_budget.js";
import { runRelatedCommand } from "./related-command-runtime.js";
import { validate } from "./validate_goose_plugin.js";
import { validateAgentPluginSchema } from "./validate_agent_plugin_schema.js";
const HERE = dirname(fileURLToPath(import.meta.url)), ROOT = portableSkillPath("assets/golden-e2e"), IDS = ["idea-api-review-skill", "dependency-review-agent", "multi-component-safety-plugin"];
...
const hash = (v) => createHash("sha256").update(v).digest("hex"), read = (p) => readFileSync(p, "utf8"), fixturePath = (id) => join(ROOT, id, "fixture.json"), expectedPath = (id) => join(ROOT, id, "expected-contract.json"), embedded = (id) => EMBEDDED_GOLDEN[id];
const fixtureRaw = (id) => import.meta.url.includes("/$bunfs/") ? embedded(id).fixtureRaw : read(fixturePath(id));
const expectedValue = (id) => import.meta.url.includes("/$bunfs/") ? embedded(id).expected : JSON.parse(read(expectedPath(id)));
...
export function loadGoldenJourney(id) { if (!IDS.includes(id))
    throw Error("unknown golden journey: " + id); const raw = fixtureRaw(id), v = (import.meta.url.includes("/$bunfs/") ? embedded(id).fixture : JSON.parse(raw)), e = expectedValue(id); if (v.schema_version !== "1.0" || v.id !== id || !v.natural_language_request || !v.scenario_suite.length)
...
function execValidator(validator, name, args) { const result = runRelatedCommand(name, args, () => ({ status: 0, stdout: "generated candidate structure validated in-process\n", stderr: "" })); const detail = (result.stdout + result.stderr).trim() || `validator exited ${String(result.status)}`; return { validator, valid: result.status === 0, detail, command: name, entry: "contained/in-process", args, exit_code: result.status, executed: true }; }
function validateCandidate(f, root) { const checks = []; if (f.artifact_kind === "skill")
    checks.push(execValidator("skill-creator/quick_validate", "skill-creator", ["validate", join(root, "skills/api-review")]));
else if (f.artifact_kind === "agent")
    checks.push(execValidator("agent-creator/validate_agent", "agent-creator", ["validate", join(root, "dependency-review.md"), "--require-filename-match"]));
else {
    const structural = validate(root), schema = validateAgentPluginSchema(root, "auto", "strict-authoring");
    checks.push({ validator: "plugin-creator/validate_goose_plugin", valid: structural.errors.length === 0, detail: [...structural.errors, ...structural.warnings].join("\n") || "plugin structure valid", command: "in-process", entry: "validate_goose_plugin", args: [root], exit_code: structural.errors.length ? 1 : 0, executed: true });
    checks.push({ validator: "plugin-creator/validate_agent_plugin_schema", valid: schema.valid, detail: schema.errors.map(x => x.message).join("\n") || "plugin schema valid", command: "in-process", entry: "validate_agent_plugin_schema", args: [root], exit_code: schema.valid ? 0 : 1, executed: true });
    checks.push(execValidator("agent-creator/validate_agent", "agent-creator", ["validate", join(root, "agents/safety-review.md"), "--require-filename-match"]));
...
async function child(_command, args, _input, signal, onHeartbeat, _heartbeatMs) { if (signal?.aborted)
    throw new Cancelled(); const started = process.hrtime.bigint(); onHeartbeat(); await new Promise(resolve => setImmediate(resolve)); if (signal?.aborted)
    throw new Cancelled(); const [mode, ...rest] = args, result = await runGoldenBehavior(mode, rest), stdout = JSON.stringify(result.output), duration_ms = Number(process.hrtime.bigint() - started) / 1e6; return { status: result.status, stdout, stderr: "", duration_ms, telemetry: normalizeRunTelemetry({ actual_turns: null, wall_time_ms: duration_ms, tokens: { input: null, output: null, cached: null, reasoning: null, total: null }, cost: null }, "golden-in-process") }; }
...
export function verifyGoldenArchive(workspace) { const root = resolve(workspace), state = JSON.parse(read(join(root, "golden-state.json"))), archiveRoot = join(root, "ci-archive"), inventory = JSON.parse(read(join(archiveRoot, "inventory.json"))), expected = JSON.parse(read(join(archiveRoot, "expected-contract.json"))), archived = JSON.parse(read(join(archiveRoot, "result.json"))), errors = []; for (const x of inventory.files) {
    ...
    errors.push("candidate provenance mismatch"); if (state.fixture_sha256 !== hash(fixtureRaw(state.journey)))
...
export async function runGoldenJourney(o) {
    const f = loadGoldenJourney(o.journey), workspace = resolve(o.workspace), statePath = join(workspace, "golden-state.json"), events = join(workspace, "golden-events.jsonl"), baseTime = o.now, fixtureBytes = fixtureRaw(f.id), fixtureSha = hash(fixtureBytes), profile = o.profile ?? "novice", configuration = config(f, profile, o.overrides ?? {}), inputSha = hash(stable({ request: f.natural_language_request, profile, configuration })), heartbeatMs = o.heartbeatMs ?? 25;
