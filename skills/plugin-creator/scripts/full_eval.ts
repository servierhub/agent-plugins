...
export interface FullEvalOptions { pluginPath:string; workspace?:string; componentReceipts?:string[]; integration?:string; archive?:string; testsStatus?:string; humanReview?:string; minPassRate?:number; minDelta?:number; dryRun?:boolean; resume?:boolean; }
interface Phase { name:string; status:"planned"|"pass"|"fail"|"blocked"|"skipped"; detail:string; }
...
export function fullEval(options:FullEvalOptions){
 ...
 const manifest=(()=>{try{return JSON.parse(requireText(join(root,"plugin.json")));}catch{return null;}})();
 ...
 const receiptByName=new Map<string,string>();
 ...
 if(options.dryRun)return envelope("planned",0,root,workspace,archive,components,phases,next_actions,null,true,Boolean(options.resume));
 const schema=validateAgentPluginSchema(root), structural=validate(root), valid=schema.valid&&!structural.errors.length;
 phases[0].status=valid?"pass":"fail"; phases[0].detail=valid?"validation passed":"validation failed";
 ...
 for(const component of missing)next_actions.push(component.command+" # writes "+component.receipt);
 const benchmark=join(integration,"benchmark.json"),review=join(integration,"review.html");
 const integrationMissing=[!existsSync(benchmark)?benchmark:null,!existsSync(review)?review:null].filter(Boolean) as string[];
 ...
 if(integrationMissing.length)next_actions.push("produce plugin integration outputs at "+q(benchmark)+" and "+q(review));
 if(!reviewPassed)next_actions.push("review "+q(review)+" then rerun with --human-review pass --resume");
 const prerequisites=valid&&!missing.length&&!integrationMissing.length&&reviewPassed;
 if(prerequisites){mkdirSync(dirname(archive),{recursive:true});const packaged=packagePlugin(root,archive);phases[4].status=packaged.status===0?"pass":"fail";phases[4].detail=packaged.status===0?archive:(packaged.stderr||packaged.stdout||"packaging failed").trim();}
 else {phases[4].status="skipped";phases[4].detail="prerequisites incomplete";}
 ...
 phases[5].status=receipt.status==="na"?"skipped":receipt.status;phases[5].detail="release verification: "+receipt.status;
 for(const [gate,value] of Object.entries(receipt.gates) as [string,{status:Status;reason?:string}][])if(value.status==="blocked"&&!next_actions.some(a=>a.includes(value.reason??"~~~")))next_actions.push("resolve "+gate+" gate: "+(value.reason??"evidence missing"));
...
function envelope(status:string,exit_code:number,plugin:string,workspace:string,archive:string,components:unknown[],phases:Phase[],next_actions:string[],verification:unknown,dry_run:boolean,resume:boolean){return{schema_version:"1.0",command:"full-eval",status,exit_code,dry_run,resume,plugin,workspace,archive,phases,components,next_actions,verification};}
