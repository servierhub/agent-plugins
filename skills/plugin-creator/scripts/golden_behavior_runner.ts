...
async function evaluate(args:string[]){
...
function challenge(args:string[]){
...
try{if(mode==="evaluate")await evaluate(args);else if(mode==="challenge")challenge(args);else throw Error("usage: golden_behavior_runner <evaluate|challenge> ...")}catch(error){console.error((error as Error).message);process.exitCode=2}
